...
def DFS_depth(graph, lefted, visited, queue , c , ans):
    visited[c] = True
    queue.append(c)
    sons = []
    son = graph.getFirst(c)
    while( son[1] != -1):
        sons.append(son)
        son = graph.getNext(c,son[1])
    del son
    for son in sons:
        if not visited[son[0]]:
            DFS_depth(graph, lefted , visited, queue, son[0], ans)
    ans[0] = ans[0] if ans[0] > len(queue) else len(queue)
...
if __name__ == "__main__":
    ...
    queue = list()
    ans = [0]
    DFS_depth(graph,dict(), visited, queue,row[start_i],ans)
